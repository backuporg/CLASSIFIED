package growthcraft.fishes.common.block;
...
{
	private final float chance = GrowthCraftFishes.getConfig().fishTrapCatchRate;
	...
	private void doCatch(World world, BlockPos pos, Random random, TileEntityFishTrap te, boolean debugFlag)
	{
		...
		boolean flag;
		if (GrowthCraftFishes.getConfig().useBiomeDict)
		{
		...
		{
			flag = Utils.isIDInList(world.getBiomeGenForCoords(pos).biomeID, GrowthCraftFishes.getConfig().biomesList);
		}
		if (flag)
		{
			f *= 1 + (75 / 100);
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		if (!world.isRemote)
		{
			player.openGui(GrowthCraftFishes.instance, 0, world, pos.getX(), pos.getY(), pos.getZ());
		}
		return true;
	}
